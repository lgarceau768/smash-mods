...
def main() -> int:
    ...
    ap.add_argument("--write-ids", help="comma-separated GameBanana ids to write, instead of --limit's top-N")
    ...
    if args.write_ids:
        ...
        chosen = [r for r in keep if r["_idRow"] in wanted]
    elif args.write:
    ...
    lines = [
        "",
        "# ---- added-character picks from scripts/curate.py ----",
        "# GameBanana Movesets category, filtered to submissions with a live",
        "# download, not flagged obsolete, and updated in the Smashline 2 era (2024+).",
        "#",
    ...
    existing = roster_path.read_text() if roster_path.exists() else ""
    roster_path.write_text(existing.rstrip("\n") + "\n" + "\n".join(lines) if existing.strip() else "\n".join(lines))
    ok(f"wrote {len(entries)} mods to roster.toml")
    return 0
